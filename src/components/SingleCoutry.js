...
export default function SingleCoutry() {
    ...
    useEffect(() => {
        const getSingleCountry = async() => {
            try {
                const API_Single_Respond = await fetch(`https://restcountries.com/v3.1/alpha/${ccn3}`);
                const API_Single_Data = await API_Single_Respond.json();
                setcountry(API_Single_Data);
            } catch (error) {
                console.log(error)
            }
        }
